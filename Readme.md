...
A [aplicação](https://github.com/Pegasus77-Adriel/Gerenciamento-de-Sensores-Inteligentes/blob/main/aplicacao/app.py) tem como objetivo fazer o gerenciamento dos sensores, utilizando-se do servidor como intermediário entre as duas partes (sensor e aplicação). Por conta disso, a aplicação é capaz de realizar comandos de ligar ou desligar um sensor facilmente atráves da requisições enviadas ao servidor, de mesmo modo que consegue receber remotamente de um sensor, indincando a apenas a matricula.
